...
parameters {
  ...
  real<lower=0>  a_culmesocore;        // upper asymptote
...
model {
  ...
  vector[N_cul] mu_scaledRI_cul = b_culmesocore 
    + (a_culmesocore - b_culmesocore) ./ pow(1 + Q_culmesocore * exp(-k_culmesocore * (t_cul - center_culmesocore)), 1 / v_culmesocore);

  vector[N_meso] mu_scaledRI_meso = b_culmesocore 
    + (a_culmesocore - b_culmesocore) ./ pow(1 + Q_culmesocore * exp(-k_culmesocore * (t_meso - center_culmesocore)), 1 / v_culmesocore);

  vector[N_crtp] mu_scaledRI_crtp = b_culmesocore 
    + (a_culmesocore - b_culmesocore) ./ pow(1 + Q_culmesocore * exp(-k_culmesocore * (t_crtp - center_culmesocore)), 1 / v_culmesocore);
