...
model {
  ...
  vector[N_cul] mu_scaledRI_cul = b_culmesocore 
    + (a_culmesocore - b_culmesocore) ./ pow(1 + Q_culmesocore * exp(-k_culmesocore * (t_cul - t0_culmesocore)), 1 / v_culmesocore);

  vector[N_meso] mu_scaledRI_meso = b_culmesocore 
    + (a_culmesocore - b_culmesocore) ./ pow(1 + Q_culmesocore * exp(-k_culmesocore * (t_meso - t0_culmesocore)), 1 / v_culmesocore);

  vector[N_crtp] mu_scaledRI_crtp = b_culmesocore 
    + (a_culmesocore - b_culmesocore) ./ pow(1 + Q_culmesocore * exp(-k_culmesocore * (t_crtp - t0_culmesocore)), 1 / v_culmesocore);
