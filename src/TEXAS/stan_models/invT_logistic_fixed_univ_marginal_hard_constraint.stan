// ═══════════════════════════════════════════════════════════════════════════════
// invT_logistic_fixed_univ_marginal_hard_constraint.stan
//
...
//   θ = {T₀, k, b, σ}. We marginalize (integrate) over this uncertainty:
//
...
//   {T₀_m, k_m, b_m, σ_m} are all indexed by m in the inner loop.
...
model {
    ...
    for (n in 1:N) {
        ...
        for (m in 1:M) {
            // Compute expected RI using the m-th calibration curve (standard logistic).
