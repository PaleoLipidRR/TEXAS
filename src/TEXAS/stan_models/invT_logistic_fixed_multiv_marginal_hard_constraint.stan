// ═══════════════════════════════════════════════════════════════════════════════
// invT_logistic_fixed_multiv_marginal_hard_constraint.stan
//
...
//          Standard logistic curve (Q = 1, ν = 1 fixed — no asymmetry or shape
//          parameters). Multivariate counterpart of the univ logistic model.
//
// CURVE: Standard logistic (special case of Richards/generalized logistic):
//   f(T; θ_m) = b_m + corrections_m + (1 - b_m) / (1 + exp(-k_m · (T - T₀_m)))
//   This is equivalent to the Richards curve with Q=1, ν=1 (symmetric sigmoid).
//   Use the gen_logi variant if Q and ν are estimated in the forward model.
//
...
functions {
  ...
  real ll_chunk(
    ...
    for (i in 1:n_chunk) {
      ...
      for (m in 1:M) {
        // Base thermal term: standard logistic evaluated at T = T_seg[i].
        ...
        // Complete the standard logistic curve (Q=1, ν=1):
        //   mu = b[m] + corrections + (1 - b[m]) / (1 + exp(-k·(T - T₀)))
...
data {
    ...
    // from the forward posterior: {T₀_m, k_m, b_m, β_m, σ_m}.
