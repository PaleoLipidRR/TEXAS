...
//   internally: t_est = min_temp + exp(η). The resulting Jacobian log(t-min_temp)
//   is automatically added to the log-target, making the effective prior:
//     p_eff(t) ∝ Normal(t | μ, σ) × (t − min_temp)
...
//
//     p_lower = Φ((min_temp − μ_n) / σ)          // CDF at lower bound
//     t_n = Normal_QF(p_lower + (1−p_lower)·q_n, μ_n, σ)
//
...
data {
    ...
    vector[M] sigma_proxy_param;  // σ_m: residual calibration noise

    // ─── Truncated Normal lower bound ─────────────────────────────────────────
    // Physical lower bound for the truncated Normal prior on temperature.
    // Typical value: -1.8°C (seawater freezing point).
...
parameters {
    ...
    // q[n] ∈ (0,1) is the quantile of the truncated Normal for sample n.
...
transformed parameters {
    ...
    // Map each q[n] ∈ (0,1) to a temperature via the quantile function of the
    // truncated Normal(prior_mu_t[n], prior_sigma_t, lower=min_temp).
    //
    //   p_lower = Φ((min_temp − μ_n) / σ)     // fraction of Normal mass below min_temp
    //   t[n] = μ_n + σ · std_normal_qf(p_lower + (1−p_lower)·q[n])
    //
    // By construction: t[n] ≥ min_temp for all n. The prior on t[n] implied by
    // q[n] ~ Uniform(0,1) is exactly TruncNormal(μ_n, σ, lower=min_temp).
    vector[N] t_est;
    for (n in 1:N) {
